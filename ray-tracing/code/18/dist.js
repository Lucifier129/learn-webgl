...
const getCenterByTime = (center1, center2, time1, time2, time) => {
  return _add_(center1, _mul_(_div_(_sub_(time, time1), _sub_(time2, time1)), _sub_(center2, time1)));
};
...
class MovingSphere {
  ...
  hit(ray, tmin, tmax, record) {
    ...
    let sphere = new Sphere(center, this.radius, this.material);
    ...
    for (let b = start; b < end; b++) {
      ...
      if (vec3.length(_sub_(center, based)) > 0.9) {
        if (chooseMat < 0.8) {
          ...
          console.log('MovingSphere', MovingSphere);
